"""
Edited by Jerry Jin: run CoRide, See details at wiki
"""
...
from algo.cfun.cworker import Worker
from algo.cfun.cmanager import Manager
from algo.cfun.base import *
from algo.cfun.cworker_buffer import Episode_w, extract_episode_w
from algo.cfun.cmanager_buffer import Episode_m, extract_episode_m
...
# run example for Beijing
def running_example(algo='CHRL', training_round=140, fleet_help=False):
    ...
    with tf.variable_scope('Worker'):
        # temperature = 0.1
        worker = Worker(sess=sess, act_space=env.get_action_space_node(), obs_space=env.get_state_space_node(),
                        message_space=message_space, feature_space=env.get_feature_space(), temperature=0.1)
    with tf.variable_scope('Manager'):
        manager = Manager(sess=sess, act_space=env.get_action_space_node(), obs_space=env.get_state_space_node(),
    ...
    for iteration in range(training_round):
        ...
        while T < max_iter:
            ...
            for _node_id in central_node_ids:
                ...
                m_replay_buffer[_node_id].append_reward([env.gmv / 1000])
            ...
            for _node_id in env.get_node_ids():
                ...
                w_replay_buffer[_node_id].append_reward([env.gmv / 1000])
...
if __name__ == "__main__":
    ...
    parser.add_argument('-a', '--algo', type=str, default='CHRL', help='Algorithm Type, choices: CHRL')
    parser.add_argument('-t', '--train_round', type=int, help='Training round limit', default=140)
